...
public class HammerUsageEvent implements PlayerBlockBreakEvents.Before {
    ...
    public boolean beforeBlockBreak(World world, PlayerEntity player, BlockPos pos, BlockState state, @Nullable BlockEntity blockEntity) {
        ...
        if (mainHand.getItem() instanceof HammerItem hammer && player instanceof ServerPlayerEntity serverPlayer) {
            if (HARVESTED_BLOCKS.contains(pos))
                return true;

            for (BlockPos position : HammerItem.getBlocksToBeDestroyed(1, pos, serverPlayer)) {
                if (pos == position || !hammer.isCorrectForDrops(mainHand, world.getBlockState(position)))
                    continue;
